...
function CourseWorkspace({ course, reload }: { course: Course; reload: () => void }) {
  ...
  return <div className="course-layout">
    ...
    <main className="course-main">
      ...
      {chapterId ? testing ? <TestLoader key={chapterId} course={course} testId={chapterId} reloadCourse={reload} onBack={() => navigate(chapterId === 'final' ? undefined : chapterId)} />
...
function TestResource({ courseId, testId, reloadCourse, onBack }: { courseId: string; testId: string; reloadCourse: () => void; onBack: () => void }) {
  const { data, error, reload } = useLearningResource<Test>('/courses/' + courseId + '/tests/' + testId);
...
function TestView({ initial, courseId, onSubmitted, onBack, reload }: { initial: Test; courseId: string; onSubmitted: () => void; onBack: () => void; reload: () => void }) {
  ...
  async function start() {
    ...
    try {
      const { data } = await api.post<Attempt>('/learning/courses/' + courseId + '/tests/' + initial.testId + '/attempts');
  ...
  async function save(submit: boolean) {
    ...
    try {
      ...
      setAttempt(data); setAnswers(data.answers);
      if (submit) { onSubmitted(); window.scrollTo({ top: 0 }); }
      else setSaved('Draft saved to your account.');
    } catch (err) { setError(extractApiError(err)); }
    finally { setBusy(false); }
  }
  const scores = testScores(initial.history, initial.testId);
  return <section>
    <button className="course-text-button" onClick={onBack}>← {initial.testId === 'final' ? 'Course overview' : 'Study material'}</button>
    <p className="course-eyebrow">{initial.testId === 'final' ? 'One question from every chapter' : 'Chapter practice'}</p><h1>{initial.title}</h1>
    <p>{initial.questionCount} questions · 1 point each · no negative marking · no time limit</p>
    {scores.latest && <p>Latest {scores.latest.percent}% · Best {scores.best}%</p>}
    ...
    {!attempt ? <button className="course-button primary" disabled={busy} onClick={start}>{busy ? 'Starting…' : 'Start test'}</button>
      : attempt.submittedAt ? <>
        <div className="course-result" role="status"><strong>{attempt.correct} / {attempt.total} · {attempt.percent}%</strong><h2>{(attempt.percent ?? 0) >= 80 ? 'A strong start' : 'Keep building your understanding'}</h2><p>{recommendation(attempt.percent ?? 0)}</p><p>This short test is a practice signal, not a complete measure of mastery.</p></div>
        <button className="course-button primary" disabled={busy} onClick={start}>{busy ? 'Starting…' : 'Retry same questions'}</button>
        <p className="course-muted">Retakes use the same fixed questions. All attempts are kept until you reset course results.</p>
    ...
    {initial.history.length > 0 && <details className="course-panel"><summary>Attempt history ({initial.history.length})</summary><ol>{initial.history.map((item) => <li key={item.id}>{new Date(item.submittedAt).toLocaleString()} · {item.percent}%</li>)}</ol></details>}
  </section>;
}
